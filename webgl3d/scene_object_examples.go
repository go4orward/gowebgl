...
func NewSceneObject_ForAxes(wctx *common.WebGLContext, length float32) *SceneObject {
	...
	geometry.AddVertices([][3]float32{{0, 0, 0}, {length, 0, 0}, {0, length, 0}, {0, 0, length}})
	geometry.AddEdges([][]uint32{{0, 1}, {0, 2}, {0, 3}}) // add three edges
...
func NewSceneObject_Cylinder_Wireframed(wctx *common.WebGLContext) *SceneObject {
	...
	geometry := NewGeometry().LoadCylinder(0.5, 6, 1.0) // create a hexagon with radius 0.5, with 1st vertex at 30 degree from X axis
